...
# 8.debug.txtを作れば実行指定不要
# 9.order.csvを作れば採点出力の順番を指定できる
...
class Compiler:
    ...
    def output_code(self,cfile,fp,filetime):
        ...
        try:
            if DEBUG:
                convert = self.execute("nkf -w -S " + TEMP + cfile + ' > ' + TEMPCONV)
            else:
    ...
    def compile_code(self,cStudent,cfile,fp):
        ...
        try:
            ...
            if u'エラー' in check or 'error' in check:
                print(u'コンパイルエラーです')
                fp.write('-----コンパイルエラー!-----\r\n')
                fp.write(check)
                return True
    ...
    def execute_program(self,num,fp):
        ...
        for i in range(self.trial):
            ...
            if result.waitUpTo(TIMEOUT_SEC) == None:
                ...
                # 不正文字を読み飛ばすように変更
                fp.write(codecs.decode(output,'utf-8','ignore'))
                fp.write('\r\n')
                point = 4
...
def ex_detect(fpath,cStudent):
    ...
    if len(fname) == 0:
        return -1
    matchOBJ = re.search('([a-z]{1,2})?[0-9]{1,2}_([0-9])\.(\w+)$', fname)
...
def main(args):
    ...
    for zipfiledir in zipdirlist:
        ...
        for infomation in verify.values():
            outfile.write(infomation['timestamp'].strftime("タイムスタンプ%c\r\n"))
            try:
                # 時間の判定
                current_num = infomation['task']
                if current_num != 0:
                    if verify[str(current_num-1)]['timestamp'] < verify[str(current_num)]['timestamp']:
                        outfile.write("時間は正常です\r\n")
                    else:
                        outfile.write("不正な時間です\r\n確認お願いします\r\n")
            except KeyboardInterrupt:
                print('強制終了します')
                sys.exit(0)
            except:
                ...
                print(infomation)
                print('タイムスタンプ取得に失敗')
            if comp[ex_num].is_pptx == False:
                # コードの内容をファイルに書き出す
                comp[i].output_code(infomation['filepath'],outfile,infomation['timestamp'])
                # 対象ソースコードをコンパイル
                # コンパイルに失敗したらTrue
                if comp[current_num].compile_code(cStudent,EXPATH+infomation['filepath'],outfile):
                    saiten['ex' + str(current_num+1)] = 2
                    # 改行と改ページの挿入
                    comp[current_num].insert_newpage(outfile)
                    continue
                # 対象プログラムを実行
                point = comp[current_num].execute_program(cStudent,outfile)
                # 改行と改ページの挿入
                comp[current_num].insert_newpage(outfile)
                # 辞書テスト
                saiten['ex' + str(current_num+1)] = point
